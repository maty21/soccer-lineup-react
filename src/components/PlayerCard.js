...
export default class PlayerCard extends React.Component {
  ...
  componentDidMount() {
    ...
    mainLoop: for (let preferredPosition of this.props.player.positions) {
      ...
      if (preferredPosition === "DC") {
        if (
          this.props.occupiedPositions.find((e) => e === "DC1") === undefined
        ) {
  ...
  findClosestPosition = (preferredPosition) => {
    ...
    for (const position of keys) {
      if (
        closestPosition === -1 ||
        Math.abs(keys.indexOf(position) - positionIndex) <
          Math.abs(keys.indexOf(position) - closestPosition)
      ) {
        let isAvailable = true;
    ...
    if (
      ...
      cardCenterPos.x =
        (100 *
          (currentPos.left +
            currentPos.width / 2 -
            this.props.parentFrame.left)) /
        this.props.parentFrame.width;
      cardCenterPos.y =
        (100 *
          (currentPos.top +
            currentPos.height / 2 -
            this.props.parentFrame.top)) /
        this.props.parentFrame.height;
      ...
      for (const indicator of Object.keys(this.props.tactic)) {
        if (
          ...
          const activePosition = ReactDOM.findDOMNode(this).dataset
            .activePosition;
          ...
          if (!isAvailable) {
            ...
            const cardToMove = document.querySelector(
              `[data-active-position='${indicator}']`
            );
  ...
  render() {
    return (
      ...
      >
        ...
        <p style={{ fontSize: "10px" }}>{this.props.player.clubName}</p>
      </div>
    );
  }
}
